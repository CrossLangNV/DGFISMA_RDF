...
from dgfisma_rdf.reporting_obligations.build_rdf import D_ENTITIES, ROGraph
from dgfisma_rdf.reporting_obligations.rdf_parser import (
    SPARQLReportingObligationProvider,
    RDFLibGraphWrapper,
    SPARQLGraphWrapper,
    URIS,
)
...
class TestRDFLibGraphWrapper(unittest.TestCase):
    def test_query_get_triples(self):
        ...
        with self.subTest("keys"):
            self.assertEqual(
                {"subject", "predicate", "object"}, set(l[0].keys()), "keys per row should contain O R S."
            )

        with self.subTest("keys of value"):
            v = l[0]["subject"]
            self.assertTrue(
                {"value", "type"}.issubset(set(v.keys())), f"Value and type should be contained in {v.keys()}"
            )
...
class TestGraphWrapper(unittest.TestCase):
    ...
    def test_query_identical_pref_labels(self):
        ...
        with self.subTest("Type return value"):
            self.assertEqual(type(l_RDFLib), type(l_SPARQLWrapper), "Should return same type")

        RDFLib_0 = l_RDFLib[0]
        SPARQLWrapper_0 = l_SPARQLWrapper[0]

        with self.subTest("Type list element"):
            self.assertEqual(type(RDFLib_0), type(SPARQLWrapper_0), "Both list elements should be of same type")

        with self.subTest("List element keys"):
            self.assertEqual(
                RDFLib_0.keys(), SPARQLWrapper_0.keys(), "Both list elements should have the same dictionary keys"
            )
...
class TestSPARQLReportingObligationProvider(unittest.TestCase):
    ...
    def test_different_entities(self):
        ...
        with self.subTest("Length of list"):
            self.assertEqual(
                len(D_ENTITIES) + 1,
                len(l),
                "amount of RO sentence entities are defined beforehand. " "One extra is from general entity",
            )
    ...
    def test_distinctness(self):
        ...
        self.assertEqual(
            len(list(l_labels_distinct)), len(set(l_labels_distinct)), "distinct should contain no value twice."
        )

        self.assertEqual(
            set(l_labels), (set(l_labels_distinct)), "distinct should contain the same values, just no doubles."
        )
...
class TestGetAllFromType(unittest.TestCase):
    ...
    def test_equivalence(self):
        ...
        def check_equivalence(d0, d1, b_subtest=False):

            for has_type_uri in l_has:

                if b_subtest:
                    with self.subTest(has_type_uri):
                        self.assertEqual(d0.get(has_type_uri), d1.get(has_type_uri), "Output should be the same.")
                else:
                    self.assertEqual(d0.get(has_type_uri), d1.get(has_type_uri), "Output should be the same.")
...
class TestGetRO(unittest.TestCase):
    ...
    def test_get_all(self):
        ...
        self.assertGreater(
            self.cas_content_example.get_NUM_RO(), 0, "Sanity check: Cas content example should not be empty!"
        )
        self.assertEqual(
            self.cas_content_example.get_NUM_RO(),
            len(l_ro_uri),
    ...
    def test_get_ro_str(self):
        ...
        l_to_find = [
            "a final system should provide for the determination",
    ...
    def test_get_ro_str_context(self):
        ...
        l_to_find = [
            "belonging to the provisional period , the Board shall use the data collected",
...
class TestSPARQLReportingObligationProviderGetFilterMultiple(unittest.TestCase):
        ...
        self.l = {
            build_rdf.KEY_CHILDREN: [
                # Base sentence
                {
                    ...
                    build_rdf.KEY_CHILDREN: [
                        {build_rdf.KEY_VALUE: "a1", build_rdf.KEY_SENTENCE_FRAG_CLASS: self.L_ENT1},
                        {build_rdf.KEY_VALUE: "a2", build_rdf.KEY_SENTENCE_FRAG_CLASS: self.L_ENT2},
                    ],
                },
                # 1 key matching
                {
                    ...
                    build_rdf.KEY_CHILDREN: [
                        {build_rdf.KEY_VALUE: "a1", build_rdf.KEY_SENTENCE_FRAG_CLASS: self.L_ENT1},
                        {build_rdf.KEY_VALUE: "b2", build_rdf.KEY_SENTENCE_FRAG_CLASS: self.L_ENT2},
                        {build_rdf.KEY_VALUE: "a3", build_rdf.KEY_SENTENCE_FRAG_CLASS: self.L_ENT3},
                    ],
                },
    ...
    def test_single_filter(self):
        ...
        with self.subTest("In all"):
            ...
            self.assertEqual(
                set([self.S0, self.S1, self.S2]), set(l_ro), "Should return these specific reporting obligations"
            )
    ...
    def test_multiple_filters(self):
        ...
        with self.subTest("test 1"):
            list_filters = [(D_ENTITIES[self.L_ENT1][0], "a1"), (D_ENTITIES[self.L_ENT2][0], "a2")]
        ...
        with self.subTest("test 2"):
            list_filters = [(D_ENTITIES[self.L_ENT1][0], "a1"), (D_ENTITIES[self.L_ENT2][0], "b2")]
            l_ro = self.ro_provider.get_filter_multiple(list_filters)
    ...
    def test_duplicates(self):
        ...
        with self.subTest("Lot of duplicates"):
            ...
            list_filters_dup = [(D_ENTITIES[self.L_ENT1][0], "a1")] * 3 + [(D_ENTITIES[self.L_ENT2][0], "a2")] * 2
    ...
    def test_no_matches(self):
        ...
        with self.subTest("two different values for same key"):
            list_filters = [(D_ENTITIES[self.L_ENT2][0], "a2"), (D_ENTITIES[self.L_ENT2][0], "b2")]
            l_ro = self.ro_provider.get_filter_multiple(list_filters)
        ...
        with self.subTest("Value not in data and existing key-value"):
            list_filters = [(D_ENTITIES[self.L_ENT1][0], "a1"), (D_ENTITIES[self.L_ENT2][0], "z2")]
            l_ro = self.ro_provider.get_filter_multiple(list_filters)
    ...
    def test_non_exact_match(self):
        ...
        list_filters_substring = [(D_ENTITIES[self.L_ENT1][0], "1"), (D_ENTITIES[self.L_ENT2][0], "a2")]

        l_ro_exact = self.ro_provider.get_filter_ro_id_multiple(list_filters_substring, exact_match=True).get(URIS)

        l_ro_non_exact = self.ro_provider.get_filter_ro_id_multiple(list_filters_substring, exact_match=False).get(
            URIS
        )
    ...
    def test_non_exact_match2(self):
        ...
        list_filters_substring = [(D_ENTITIES[self.L_ENT1][0], "a"), (D_ENTITIES[self.L_ENT2][0], "a2")]
    ...
    def test_multiple_filters_ro_id(self):
        ...
        with self.subTest("test 1"):
            ...
            l_ro_GT = [
                a.get("id")
                for a in self.l.get(build_rdf.KEY_CHILDREN)
                if a.get(build_rdf.KEY_VALUE) in [self.S0, self.S2]
            ]
        ...
        with self.subTest("test 2"):
            ...
            l_ro_GT = [
                a.get("id") for a in self.l.get(build_rdf.KEY_CHILDREN) if a.get(build_rdf.KEY_VALUE) in [self.S1]
            ]
...
class TestSPARQLPagination(unittest.TestCase):
    ...
    def test_foo(self):
        ...
        limit = 10
        l_ro_uri = self.get_ro_provider().get_filter_ro_id_multiple(limit=limit, offset=0).get(URIS)
...
class TestFilterDropdown(unittest.TestCase):
    ...
    def test_return(self):
        ...
        for ent_0_i in l_ent_0:
            with self.subTest(f'Should return itself. "{ent_0_i}"'):
                l_ent_1_filtered = self.prov.get_filter_entities_from_type(type_ent0, [(type_ent0, ent_0_i)])
                    ...
                    if len(l_ent_filter_j):
                        ...
                        for type_ent_k in l_types_ent:
                            ...
                            for ent_j_j in l_ent_filter_j:
                                ...
                                l_ent_filter_k = self.prov.get_filter_entities_from_type(
                                    type_ent_k,
                                    [
                                        (type_ent_i, ent_i_i),
                                        (type_ent_j, ent_j_j),
                                ...
                                if len(l_ent_filter_k):
                                    l_ro = self.prov.get_filter_ro_id_multiple(
                                        [
                                            (type_ent_i, ent_i_i),
                                            (type_ent_j, ent_j_j),
    ...
    def test_equivalence(self):
        ...
        with self.subTest("Filtered entities"):
            ...
            check_equivalence(d_base, d1, b_subtest=True)
        ...
        for uri_type_has in l_uri_has:
            ...
            with self.subTest(uri_type_has):
                ...
                for c in string.ascii_lowercase:
                    l += self.prov.get_filter_entities_from_type_lazy_loading(
                        uri_type_has, str_match=c, type_match=rdf_parser.STARTS_WITH
                    )
        ...
        for uri_type_has in l_uri_has:
            ...
            with self.subTest(uri_type_has):
                ...
                for uri_type_has_j in l_uri_has:
                    ...
                    l_no_filter_j = self.prov.get_filter_entities_from_type_lazy_loading(
                        uri_type_has_j, list_pred_value=[(uri_type_has, v0_i)]
                    )
...
class TestSorting(unittest.TestCase):
    ...
    def test_sort_filter_ent(self):
        ...
        for ent_type_i in [ent_type_ent, ent_type_prp, ent_type_adv, ent_type_reporter, ent_type_prd, ent_type_report]:
...
class TestSortingStartsWithFirst(unittest.TestCase):
    ...
    def test_get_dropdown_items(self):
        ...
        for type_contains, keyword in {"tmp": "year", "reg": "the commission"}.items():
            with self.subTest(type_contains):
                ...
                l_type_i_filter = self.prov.get_filter_entities_from_type_lazy_loading(
                    type_i,
                    str_match=keyword,
                ...
                self.assertEqual(
                    len(l_type_i_filter), len(l_ordered_baseline), "Sanity check. Should contain same amount of items."
                )
