...
class TestGetDocIDs(unittest.TestCase):
    ...
    def test_test_amount_docs(self):
        for name, url in {'test': URL_FUSEKI,
                          'staging': URL_STAGING,
                          'production': URL_FUSEKI_PRD
                          }.items():
...
class TestGetROFilterDocIDs(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.l_doc_uri_subset = self.l_doc_uri[:(len(self.l_doc_uri) // 2)] + ['www.some_test.com',
                                                                               'http://doc_uri_s.io']
    ...
    def test_filter_entities_from_type(self):
        ...
        l_ent_all = self.prov.get_filter_entities_from_type_lazy_loading(uri_type_has,
                                                                         l_doc_uri=self.l_doc_uri)

        l_ent_subset = self.prov.get_filter_entities_from_type_lazy_loading(uri_type_has,
                                                                            l_doc_uri=self.l_doc_uri_subset)

        l_ent_none = self.prov.get_filter_entities_from_type_lazy_loading(uri_type_has,
                                                                          l_doc_uri=[
                                                                              'https://no_document_with_this_uri_should_exist.be'])
    ...
    def test_filter_ro_id_multiple(self):
        ...
        l_ro_none = self.prov.get_filter_ro_id_multiple(l_doc_uri=['https://no_document_with_this_uri_should_exist.be'])
